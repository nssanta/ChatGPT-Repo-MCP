...
def _rg_exclude_globs(settings: Settings) -> list[str]:
    ...
    for pattern in settings.blocked_globs:
        if settings.allow_secret_access and normalize_rel_path(pattern) in secret_patterns:
            continue
...
def list_dir(path: str, settings: Settings, include_hidden: bool = True, limit: int = 200) -> dict[str, Any]:
    ...
    for entry in entries:
        entry["path"] = display_path(root / entry["path"], settings)
    return {"path": context.display, "entries": entries, "truncated": len(entries) >= limit}
...
def tree(path: str, settings: Settings, depth: int = 4, include_hidden: bool = True) -> dict[str, Any]:
    ...
    lines.append(context.display)
    walk(target, "", depth)
    return {"path": context.display, "tree": "\n".join(lines), "entries": count, "max_entries": max_entries}
...
def find_files(
    ...
    for found in _iter_files(root, target, settings, allow_hidden=include_hidden):
        ...
        if fnmatch(found.name, pattern) or fnmatch(rel, pattern) or found.match(pattern):
            matches.append(display_path(root / rel, settings))
...
def recent_changes(settings: Settings, path: str = ".", paths: list[str] | None = None, limit: int = 100) -> dict[str, Any]:
    ...
    for item in search_paths:
        ...
        for file_path in _iter_files(context.root, context.target, settings, allow_hidden=settings.allow_hidden_default):
            stat = file_path.stat()
            row = {"path": display_path(file_path, settings), "mtime": stat.st_mtime, "size": stat.st_size}
            if row not in items:
                items.append(row)
    ...
    for manifest in manifests:
        ...
        try:
            if manifest.name == "pyproject.toml":
                parsed[display_path(manifest, settings)] = _parse_pyproject(manifest)
            elif manifest.name == "requirements.txt":
                parsed[display_path(manifest, settings)] = _parse_requirements_txt(manifest)
            elif manifest.name == "package.json":
                parsed[display_path(manifest, settings)] = _parse_package_json(manifest)
