...
В Go-сервере `doctor` и `context_bootstrap` возвращают краткий обзор репозиториев (`path` и `is_git`) без проверки Git-статуса каждого проекта. Если нужны ветка и состояние файлов, вызовите `list_repos`: он запускает Git-команды для каждого найденного репозитория и в большом workspace может выполняться дольше.
